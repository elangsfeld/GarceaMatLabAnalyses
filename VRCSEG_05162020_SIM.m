...
inDir = 'G:\DATA\08.02.19 EdU P-C with RA, NG18, and 808A\RA, 0 Chase\*.nd2';
Folder = 'G:\DATA\08.02.19 EdU P-C with RA, NG18, and 808A\RA, 0 Chase\';
...
DBSCAN_Rad(1,1:4) = [15 15 15 15];
DBSCAN_MinPts(1,1:4) = [50 50 50 50];

RS_Rad(1,1:4) = [40 40 40 40];

VRC_ID = [1 1 0 0]; %Chooses which channel(s) to use for VRC identification (can be up to 2).
...
if drawVRCimages == 1,
    if VRC_ID == [1,0,0,0], imshow(Ch1_seg(:,:,Ch1_MaxSlice(f,1)).*3);
    ...
    for a = 1:size(VRC_crops,1),
        rectangle('Position',[VRC_crops.VRC_MinX(a) VRC_crops.VRC_MinY(a) (VRC_crops.VRC_MaxX(a)-VRC_crops.VRC_MinX(a)) (VRC_crops.VRC_MaxY(a)-VRC_crops.VRC_MinY(a))],'EdgeColor','y'); %[x y w h]
    end
