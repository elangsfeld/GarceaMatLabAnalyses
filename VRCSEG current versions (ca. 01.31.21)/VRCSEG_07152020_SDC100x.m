...
inDir = 'C:\Users\dplad\Desktop\testttt\*.tif';
Folder = 'C:\Users\dplad\Desktop\testttt\';

srcFiles = dir(inDir);

%%% Requires INPUT %%%
LPercentileThresh(1,1:4) = [99.9 99.95 99.95 99.95];
...
DBSCAN_MinPts(1,1:4) = [10 10 10 10];
...
for m = 1:Slices
    ...
    if Channels>1, Ch2(:,:,m) = I{1,1}{Ch2_planes(m,1),1};
    Ch2_IntDen_Slice(f,m) = sum(sum(Ch2(:,:,m)));
    [Ch2_Max(f,1), Ch2_MaxSlice(f,1)] = max(Ch2_IntDen_Slice(f,:));else end
