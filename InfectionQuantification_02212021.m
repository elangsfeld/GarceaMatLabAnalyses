...
Folder = 'F:\DATA\Infectivity02192021\2021-02-19\6483\';
%Format: 'FORMAT\'

FigShow = 0; %Do you want to display an image of nuclear segmentation and TAg signal? (1=yes,0=no).
...
Analysis_TAg = 1; %Do you want to analyze TAg intensity? (1=yes,0=no).
    ...
    Threshold_TAg = 4000; %Where do you want the threshold for infected and uninfected cells to be called, based on TAg signal?
...
Analysis_VP1 = 1; %Do you want to analyze VP1 intensity? (1=yes,0=no).
    ...
    Threshold_VP1 = 15000; %Where do you want the threshold for infected and uninfected cells to be called, based on VP1 signal?
...
for f = START:FINISH
    ...
    DAPI.bg = imopen(DAPI.Image,strel('disk',round(Res/10)));
    DAPI.Image2 = DAPI.Image - DAPI.bg;
    DAPI.BW = imbinarize(DAPI.Image2);
    DAPI.BW2 = imfill(DAPI.BW,'holes');
    DAPI.BW2 = xor(bwareaopen(DAPI.BW2,10),bwareaopen(DAPI.BW2,200));
    
    DAPI.cc = bwconncomp(DAPI.BW,4);
    
    figimage1 = imoverlay(imadjust(DAPI.Image2),bwperim(DAPI.BW2),[1 0 0]);
